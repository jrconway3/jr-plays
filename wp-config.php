...
$table_prefix = getenv( 'WP_DB_PREFIX' ); // phpcs:ignore WordPress.WP.GlobalVariablesOverride.Prohibited
if ( ! $table_prefix ) {
...
if ( $jr_site_url ) {
	...
	if ( ! $jr_site_scheme ) {
		...
		if ( ! $jr_site_scheme ) {
			$jr_site_scheme = ( isset( $_SERVER['HTTPS'] ) && 'on' === $_SERVER['HTTPS'] ) ? 'https' : 'http';
		}
	}
